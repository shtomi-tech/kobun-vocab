...
古文単語の4択演習と、古典文法を学習順に進める1つの静的アプリです。段階1の必須200語（20問×10セット）を終えるまで文法をロックし、段階2は「用言 → 助動詞 → 助動詞識別 → 敬語基礎 → 文法混合確認」、段階3は「敬意の方向 → 省略主語 → 短文統合 → 敬語読解チェック」、段階4は「宮廷生活 → 恋愛・婚姻 → 年中行事 → 古文常識チェック」の順で進みます。
