...
> **現行実装（2026-07-28）**：実践問題は、`questionType: integration` を維持したまま、古文本文と傍線部を示して意味を4択で答える1問形式とする。旧 `steps` 配列とステップ実行UIは廃止した。助動詞資料から29問を優先反映し、保留していた8問には「その他の助動詞の意味」の手順と導入4択5問を追加した。以下の旧計画記述にある「統合ステップ」は、この現行仕様では「実践4択」と読み替える。

> **追記（2026-07-28・保留問題の反映）**：登録129問・必修107問へ更新した。既存の「つ・ぬ」「まし」に各1問を追加し、らし・めり・たし・ごとし・断定のたりの6問を新設した「その他の助動詞の意味」手順へまとめた。後続段階をすでに開始している利用者が新問題追加によって再ロックされないよう、進捗のある段階は引き続き表示・再開できる。
...
> **追記（2026-07-25・助詞の必修ルート）**：助詞の識別41問は、条件17問・対比4問を残し、
> 手順5問・統合9問を必修とする35問へ最適化した。残り6問は削除せず、追加練習として扱う。

> **追記（2026-07-25・同形語の必修ルート）**：同形語の識別57問は、条件23問・対比5問を残し、
> 手順5問・統合12問を必修とする45問へ最適化した。残り12問は削除せず、追加練習として扱う。

> **追記（2026-07-25・敬語の必修ルート）**：敬語の識別44問は、条件19問・対比5問を残し、
> 手順5問・統合10問を必修とする39問へ最適化した。残り5問は削除せず、追加練習として扱う。

> **追記（2026-07-25・文法の入口／文法4択の必修ルート）**：文法の入口28問は26問、文法4択90問は66問を必修とし、残りは削除せず追加練習として扱う。セットごとの必修IDをデータに持たせ、ロードマップ、進捗、混合確認の母集団を必修問題にそろえる。
...
- **kyouzai-bank 連携**：教材バンクは 2026-07-17 にアーカイブしたため、現行の連携は行わない。
